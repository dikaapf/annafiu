...
class Manajemen_user extends SIMASJID_Core {
	...
	public function index()
	{
		...
		$this->_data 	= array(
			'title' 	=> 'An-Naafiu - Manajemen User',
		...
		{	
			if($this->user_m->checkUser($post['user_name'], $post['user_email'], 'tambah') == 0)
			{
				...
				if($status !== 0) {
					...
					$this->email->subject('An-Naafiu - Aktivasi Akun');
					$this->email->message("Email anda telah terdaftar pada Aplikasi An-Naafiu " . $this->app->nama_masjid);
